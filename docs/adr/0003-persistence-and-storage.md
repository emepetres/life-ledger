...
**Implementation note (for the follow-on build, out of scope here):** the backup must produce a **consistent snapshot** — SQLite's Online Backup API or `VACUUM INTO` — not a raw file copy of a live WAL-mode database.
